...
func loadSymbolData() -> SymbolData {
    ...
    if let url = Bundle.main.url(forResource: "layerset_availability", withExtension: "plist"),
       let data = try? Data(contentsOf: url) {
        ...
        if let root = try? PropertyListSerialization.propertyList(from: data, options: [], format: &format) as? [String: Any],
    ...
    if let url = Bundle.main.url(forResource: "categories", withExtension: "plist"),
       let data = try? Data(contentsOf: url) {
        var format = PropertyListSerialization.PropertyListFormat.xml
        if let array = try? PropertyListSerialization.propertyList(from: data, options: [], format: &format) as? [[String: Any]] {
    ...
    if let url = Bundle.main.url(forResource: "symbol_categories", withExtension: "plist"),
       let data = try? Data(contentsOf: url) {
        var format = PropertyListSerialization.PropertyListFormat.xml
        if let dict = try? PropertyListSerialization.propertyList(from: data, options: [], format: &format) as? [String: [String]] {
